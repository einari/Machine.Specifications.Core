...
namespace Machine.Specifications.Factories
{
  public class ContextFactory
  {
    ...
    static Subject ExtractSubject(Type type)
    {
      ...
      var attribute = (SubjectAttribute)attributes.First();
    ...
    static void CollectDetailsOf<T>(Type target, Func<object> instanceResolver, ICollection<T> items, bool ensureMaximumOfOne, AttributeFullName attributeFullName)
    {
      ...
      if (target.GetTypeInfo().IsAbstract)
      {
      ...
      if (!IsStatic(target))
      {
        ...
        CollectDetailsOf(target.GetTypeInfo().BaseType, () => instance, items, ensureMaximumOfOne, attributeFullName);
      }
    ...
    {
      return target.GetTypeInfo().IsAbstract && target.GetTypeInfo().IsSealed;
    }
