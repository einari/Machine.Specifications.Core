...
namespace Machine.Specifications.Utility
{
  public static class RandomExtensionMethods
  {
    ...
    internal static bool HasAttribute<TAttributeFullName>(this Type attributeProvider, TAttributeFullName attributeFullName)
      where TAttributeFullName : AttributeFullName
    {
      var attributeType = Type.GetType(attributeFullName.FullName);
      return attributeProvider.GetTypeInfo().GetCustomAttributes(attributeType, true).Any();
    }

    internal static bool HasAttribute<TAttributeFullName>(this FieldInfo attributeProvider, TAttributeFullName attributeFullName)
      where TAttributeFullName : AttributeFullName
    {
      var attributeType = Type.GetType(attributeFullName.FullName);
    ...
    internal static AttributeFullName GetCustomDelegateAttributeFullName(this Type type)
    {
      ...
      if (type.GetTypeInfo().GetCustomAttributes(attributeType, false).Any())
      {
