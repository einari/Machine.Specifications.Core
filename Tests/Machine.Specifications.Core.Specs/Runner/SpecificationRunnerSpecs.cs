...
namespace Machine.Specifications.Specs.Runner
{
  ...
  {
    Because of = () =>
      Run<context_with_trace_output>();

    It should_capture_the_debug_trace =
      () => testListener.LastAssembly.CapturedOutput.Should().Be(String.Format("Trace.WriteLine message in establish{0}" +
                                                                               "Trace.WriteLine message in because{0}" +
                                                                               "Trace.WriteLine message in spec{0}" +
                                                                               "Trace.WriteLine message in nth spec{0}" +
                                                                               "Trace.WriteLine message in cleanup{0}",
                                                                               Environment.NewLine));
  }
